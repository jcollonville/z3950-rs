...
async fn start_yaz_server(port: u16) -> std::result::Result<std::process::Child, Error> {
    let yaz_server_path = std::env::var("YAZ_SERVER_PATH").unwrap_or_else(|_| "yaz-ztest".to_string());

    let mut cmd = Command::new(&yaz_server_path);
    cmd.arg(format!("tcp:localhost:{}", port)).stdout(Stdio::piped()).stderr(Stdio::piped());
...
async fn test_delete_result_sets() {
    ...
    assert!(
        matches!(response.delete_operation_status, DeleteOperationStatus::Success | DeleteOperationStatus::ResultSetDidNotExist),
...
async fn test_scan() {
    ...
    assert!(
        matches!(response.scan_status, ScanStatus::Success | ScanStatus::PartialBeginning | ScanStatus::PartialEnd | ScanStatus::Failure),
...
async fn test_sort() {
    ...
    // Create sort keys
    let sort_keys = vec![Client::sort_key_by_field("title", true, false)];
    ...
    if let Ok(response) = sort_result {
        ...
        assert!(
            matches!(response.sort_status, SortStatus::Success | SortStatus::PartialResultsAvailable | SortStatus::Failure),
...
async fn test_close() {
    ...
    assert!(
        matches!(
            close_pdu.close_reason,
            CloseReason::Finished
                | CloseReason::Shutdown
                | CloseReason::SystemProblem
                | CloseReason::CostLimit
                | CloseReason::Resources
                | CloseReason::SecurityViolation
                | CloseReason::ProtocolError
                | CloseReason::LackOfActivity
                | CloseReason::PeerAbort
...
async fn test_extended_services() {
    ...
    let ext_result = client.extended_services(ExtendedServicesFunction::Create, oid, Some("test_package"), None, WaitAction::DontWait).await;
    ...
    if let Ok(response) = ext_result {
        ...
        assert!(
            matches!(
                ...
                ExtendedServicesStatus::Done | ExtendedServicesStatus::Accepted | ExtendedServicesStatus::Failure
            ),
...
async fn test_duplicate_detection() {
    ...
    if let Ok(response) = dup_result {
        ...
        assert!(
            matches!(
                response.status,
                DuplicateDetectionStatus::Success
                    | DuplicateDetectionStatus::PartialSomeInputNotProcessed
                    | DuplicateDetectionStatus::PartialSomeCriteriaNotApplied
                    | DuplicateDetectionStatus::PartialBothProblems
                    | DuplicateDetectionStatus::Failure
            ),
    ...
    if let Ok(response) = report_result {
        ...
        assert!(
            matches!(
                ...
                ResourceReportStatus::Success
                    | ResourceReportStatus::Partial
                    | ResourceReportStatus::FailureNoReport
                    | ResourceReportStatus::FailureNoReportNoEstimate
                    | ResourceReportStatus::FailureNoReportCannotDetermine
                    | ResourceReportStatus::FailureNoReportDueToCondition
                    | ResourceReportStatus::FailureReportNotSupported
                    | ResourceReportStatus::FailureReportFormatNotSupported
            ),
